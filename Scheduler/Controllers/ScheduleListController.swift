...
class ScheduleListController: UIViewController {
  ...
  @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
    ...
    private func showCreateEventVC(_ event: Event? = nil){
        // we made it optional so that way its required to pass in an arguement
        //then you give it a defauly parameter in case so that way the user doesnt have to...
        // so when you give it a default you do not have to put something inside of it when its being implimented
        // need to use the storyboard to get an instance of the create event controller
        // all viewcontroller have a property called storyboard on them
        guard let createEventController = storyboard?.instantiateViewController(identifier: "CreateEventController") as? CreateEventController else {
            // need to type cast because we need to get the view controller
            fatalError("couldn't downcast to CreateEventController ")
        }
        // TODO:
        ...
        //if let event = event { // means an event was passed into this function
            // double checks if it has a value..
            // didselect.. alreay created
            //
        createEventController.event = event
...
extension ScheduleListController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // if you type it has an options to not use an event...
        showCreateEventVC(selectedEvent)
    }
